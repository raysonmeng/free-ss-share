...
import requests
import threading
from free_ss_share.settings.development import HEADERS, URL_LIST, PROXIES
...
def test(link):
    ...
    os.system('ss-local -c /etc/shadowsocks.json -f /etc/ss.pid')
    print '|    打开客户端！'
    time.sleep(3)
    thread_list = []
    for url in URL_LIST:
    ...
    for thread in thread_list:
        if thread.isAlive():
            thread.join()
    print '|    网速测试结束。'
    # os.system('python /home/shadowsocksr/shadowsocks/local.py -c /etc/shadowsocks.json -d stop -q')
    os.system('kill -9 `cat /etc/ss.pid`')
    print '|    关闭客户端！'
    time.sleep(3)
...
if __name__ == '__main__':
    test(
        'ssr://MTA0LjE2MC4xODUuODA6NDcwNzI6YXV0aF9zaGExX3Y0OmFlcy0yNTYtY2ZiOnRsczEuMl90aWNrZXRfYXV0aDpaRzl1WjNSaGFYZGhibWN1WTI5dC8/cmVtYXJrcz01cHlzNkxTbTVZLTM1cDJsNkllcU9tUnZkV0l1YVc4dmMzTjZhR1o0')
